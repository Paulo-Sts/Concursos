Aqui estão as anotações detalhadas sobre **Banco de Dados – Tipos de Usuários, Linguagens e Arquitetura SGBD**, mantendo o estilo dos resumos anteriores.

---

# Anotações – Banco de Dados – Tipos de Usuários, Linguagens e Arquitetura SGBD
...
> ### 1.5.1 DBA vs. Administrador de Dados – Quadro comparativo
...
### 2.1 Visão geral das linguagens
...
> - A linguagem SQL **combina VDL, DDL e DML**.
> - É a linguagem padrão para bancos de dados relacionais.

### 2.2 Exemplo de DDL – CREATE TABLE
...
> ### 2.2.1 Importância das restrições
...
### 2.4 DML Procedural vs. Não Procedural
...
| CONCEITO | DEFINIÇÃO |
| --- | --- |
| **DBA** | Administra o SGBD (monitoramento, segurança, backup). |
| **Administrador de Dados** | Administra os dados em si (padronização, integração, modelagem conceitual). |
| **DDL** | Define estrutura do banco de dados (CREATE, ALTER, DROP). |
| **DML** | Manipula dados (SELECT, INSERT, UPDATE, DELETE). |
| **DML Procedural** | Especifica "o que" e "como" obter. |
| **DML Não Procedural** | Especifica apenas "o que" obter (ex.: SQL padrão). |
| **Metadados** | Dados sobre os dados (armazenados no catálogo). |
| **Modelagem conceitual** | Define "o que" será armazenado, sem preocupação com implementação. |
