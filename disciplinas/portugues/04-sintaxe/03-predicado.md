# Predicado

## 1. Definição
- O predicado é a parte da oração que contém o verbo e que atribui uma ação, estado ou qualidade ao sujeito. 
- O verbo é o elemento central do predicado, e a classificação do predicado depende do tipo de verbo e de como ele interage com o sujeito e os complementos.
- Em uma oração, o predicado sempre está associado ao sujeito e, em conjunto, expressam a ideia completa da ação ou estado descrito. 

## 2. Tipos de Predicado

### 2.1 Predicado Verbal 
- O verbo principal (núcleo) é um verbo de ação e não tem verbo de ligação.
- Um verbo de ação indica algo que o sujeito faz.
- Exemplo: 
  - O aluno **estudou** muito para a prova. (O verbo "estudou" é o núcleo do predicado e expressa a ação praticada pelo sujeito.)

### 2.2 Predicado Nominal 
- O verbo principal (núcleo) é um verbo de ligação e liga o sujeito a uma característica ou estado (predicativo do sujeito), não possuindo verbo de ação.
- Verbo de ligação é um verbo que conecta o sujeito a uma característica ou estado, não exercendo nenhuma ação.
- Exemplo: 
  - O céu **está** claro. (O verbo "está" é o núcleo do predicado e faz a ligação entre o sujeito "o céu" e o predicativo "claro".)

### 2.3 Predicado Verbo-nominal
- Combina um verbo de ação e um predicativo. O predicativo pode caracterizar o sujeito ou o objeto (predicativo do sujeito ou objeto).
- Exemplo: 
  - O professor **chegou** cansado. (O verbo "chegou" é o núcleo verbal, e o predicativo "cansado" caracteriza o sujeito "o professor".)

### Tabela Comparativa

| TIPO DE PREDICADO | DEFINIÇÃO                                                                                       | EXEMPLO                   | NÚCLEO DO PREDICADO |
|-------------------|-------------------------------------------------------------------------------------------------|---------------------------|---------------------|
| Verbal            | O núcleo do predicado é um verbo de ação e não há verbo de ligação.                             | Ele estudou para a prova. | estudou             |
| Nominal           | O núcleo do predicado é um verbo de ligação, e o sujeito recebe uma característica              | O céu está azul.          | está                |
| Verbo-nominal     | O núcleo do predicado é um verbo de ação e há um predicativo, que descreve o sujeito ou objeto. | Ela chegou cansada.       | chegou              |

> [!TIP] DICAS:
> Identificação do Predicado:
> 1. Localizar o verbo.
> 2. Achar o sujeito fazendo a seguinte pergunta ao verbo: Quem? ou O que? + verbo
> 2. Identificar o papel do verbo (Verbo de ação: Indica que algo aconteceu. Verbo de ligação: usado para ligar uma característica ao sujeito.)
> 3. Procurar se no predicado há alguma caracterização do sujeito, como uma qualidade ou estado.
> 4. Classificar o predicado. (Predicado verbal: apenas verbo de ação. Predicado nominal: verbo de ligação + característica. Predicado verbo-nominal: verbo de ação + característica).

#### Exemplos de Verbos de Ação

| AÇÕES | VERBOS | 
|---|---|
| Físicas   | Correr, andar, cair, cantar, comer...  | 
| Mentais   | Pensar, imaginar, decidir, aprender... | 
| Fenômenos | Chover, ventar, nevar...               | 

#### Exemplos de Verbos de Ligação

| VERBO | SIGNIFICADO | EXEMPLO |
|---|---|---|
| Ser                  | Indica estado permanente    | Ela é inteligente.    |
| Estar                | Indica estado transitório   | Ele está cansado.     | 
| Ficar                | Indica mudança de estado    | Fiquei surpreso.      |
| Parecer              | Indica estado aparente      | O dia parece chuvoso. |
| Continuar/Permanecer | Indicam continuidade        | Ela continua firme.   |
| Tornar-se/Virar      | Indicam transformação       | Ele tornou-se médico. |
| Andar/Viver          | (quando usados como estado) | Ele anda preocupado.  |

> [!CAUTION] OBSERVAÇÃO:
> - Um mesmo verbo pode ser de ação ou de ligação dependendo de como é usado na frase.
> - Exemplo:
>   - Ação: "O menino andou pela rua." (indica o ato físico de caminhar)
>   - Ligação: "O menino andou triste." (indica o estado emocional do menino)
